...
pub(crate) static SEMAPHORE: Lazy<Arc<Semaphore>> =
    Lazy::new(|| Arc::new(Semaphore::new(MAX_POOL_SIZE)));

// Pool of sandboxes, initialized lazily
pub(crate) static MULTI_USE_SANDBOX_POOL: Lazy<Arc<Mutex<Vec<Arc<Mutex<MultiUseSandbox>>>>>> =
    Lazy::new(|| Arc::new(Mutex::new(Vec::with_capacity(MAX_POOL_SIZE))));
